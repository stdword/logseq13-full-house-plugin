  ...
  */
  parseTags: {
    '``{...}``': {
      ...
      compileFunction: (content) => (content + '\n'),
    },
    '``[...]``': {
      trimRight: false,
      autoFilter: true,
      compileFunction: (content) => `ref(${content})`,
    },
    '``...``': {
      trimRight: false,
      autoFilter: true,
      parseFunction: (content) => ({protectValue: true}),
    },
    // '<%...%>': {
    //   trimRight: false,
    //   autoFilter: false,
...
function compileBody(buff) {
  ...
  for (i; i < buffLength; i++) {
    ...
    if (typeof currentBlock === "string") {
      ...
      const { parseFunction, compileFunction, filterFunction, protect } = parseInfo;
      ...
      if (compileFunction)
        content = compileFunction(content);
